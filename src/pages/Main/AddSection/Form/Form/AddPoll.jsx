            ...
            <div className="w-[390px] h-[80vh] flex flex-col items-center relative z-[0]">
                <div className='px-4 my-[50px] flex w-full justify-between'>
                    <div className='flex items-center gap-x-[16px]'>
                        <span className='text-white'>Add a poll</span>
                    </div>
                    ...
                    <div className='w-full'>
                        ...
                        {inputFields.map((inputField, index) => (
                            <div key={index} className='p-4 flex w-full justify-between items-center' style={{ fontSize: '12px' }}>
                                <DizeeInput2
                                    label={`Field ${index + 1}`}
                                    placeholder={`Field selection ${index + 1}`}
                                    className="dizee-input w-full text-white"
                                    value={inputField}
                                    onChange={event => handleInputChange(index, event)}
                                />
                                {(index !== 0 && index !== 1) && <Close className='cursor-pointer h-[14px] w-[14px] text-white' onClick={() => handleRemoveField(index)} />}
                            </div>
                        ))}

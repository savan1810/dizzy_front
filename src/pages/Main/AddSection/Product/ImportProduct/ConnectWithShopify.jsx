...
export default function ConnectWithShopify() {
    ...
    const debouncedApiCall = useCallback(
        _.debounce(async (value) => {
            ...
            try {
                dispatch(clearAlerts());
                const response = await getDataAPI(`user/install?shopUrl=${value}`, token);
    ...
    return (
        <LayoutHeader>
            <div className="w-[350px] sm:w-[390px] h-[80vh] bg-black flex flex-col  items-center relative">
                <div className='px-4 mt-[50px] mb-[200px] flex w-full justify-between items-start'>
                    <div className='flex items-center gap-x-[16px]'>
                        {/* {IconComponent} */}
                        <span className='text-white'>Import product link</span>
                    </div>
                    <Link to="/add-section/add-product">

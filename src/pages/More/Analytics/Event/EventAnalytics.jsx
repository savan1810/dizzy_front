...
export default function EventAnalytics() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                await getDataAPI(`analytics/get-event-analytics?filter=${filter}`, token).then((res) => {
        ...
        <LayoutHeader>
            <div className="w-[390px] flex flex-col justify-start items-center relative z-[0]">
                <div className='px-4 my-[50px]  flex w-full justify-between'>
                    <p className='text-white'>Event analytics</p>
                ...
                <div className='px-4 my-[50px] flex w-full justify-between'>
                    ...
                    <p className='text-white cursor-pointer' onClick={() => navigate('/more/analytics/filter', { state: { path: '/more/analytics/event' } })}>Filter</p>
                </div>
                    ...
                    </div>
                    {
                        data?.length > 0 && data.map((item, index) => {
                            return (<div className='flex justify-between items-center'>
                                <p className='text-white'>{maxLength(item?.venue, 30)}</p>

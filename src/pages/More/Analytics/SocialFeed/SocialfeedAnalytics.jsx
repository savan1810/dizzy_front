...
export default function SocialFeedAnalytics() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                await getDataAPI(`analytics/get-socialfeed-analytics?filter=${filter}`, token).then((res) => {
        ...
        <LayoutHeader>
            <div className="w-[390px] flex flex-col justify-start items-center relative z-[0]">
                <div className='px-4 my-[50px]  flex w-full justify-between'>
                    <p className='text-white'>Social feed analytics</p>
                ...
                <div className='px-4 my-[50px] flex w-full justify-between'>
                    ...
                    <p className='text-white cursor-pointer' onClick={() => navigate('/more/analytics/filter', { state: { path: '/more/analytics/socialfeed' } })}>Filter</p>
                </div>
                <div className='px-4 my-[50px] flex flex-col gap-y-[38px] w-full '>
                    <div className='flex justify-between items-center'>
                        <p className='text-white'>All platforms</p>
                        <p className='text-white'>{totlaClick}</p>
                    </div>
                    {
                        data?.length > 0 && data.map((item, index) => {
                            return (<div className='flex justify-between items-center'>
                                <p className='text-white'>{maxLength(item?.platform, 30)}</p>

...
export default function PollsAnalytics() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                await getDataAPI(`analytics/get-poll-analytics?filter=${filter}`, token).then((res) => {
        ...
        <LayoutHeader>
            <div className="w-[390px] flex flex-col justify-start items-center relative z-[0]">
                ...
                <div className='px-4 my-[50px] flex w-full justify-between'>
                    ...
                    <p className='text-white cursor-pointer' onClick={() => navigate('/more/analytics/filter', { state: { path: '/more/analytics/polls' } })}>Filter</p>
                </div>

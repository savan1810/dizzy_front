...
export default function Playlist(props) {
    ...
    const handlePlaylistClick = async (item) => {
        ...
        try {
            await postDataAPI('analytics/create-playlist-analytics', { link: item?.links[0]?.url, domain: dynamicVar, title: item?.title }, null);
        }
        catch (err) {
            console.log(err)
        }
                        ...
                        <>
                            <div className='flex w-full item-center justify-between px-4'>
                                ...
                                <div className='flex gap-x-1'
                                    onClick={() =>
                                        window.open(`/playlist-data/${dynamicVar}`, '_blank')
                                    }

        ...
        <>
            {
                ...
                <div className='fixed top-0 left-0 w-full h-[100px] flex items-center justify-between px-[3vw] z-[9] bg-black'>
                    ...
                    <div className='flex items-center gap-[12px] sm:gap-[100px] md:gap-[120px] lg:gap-[220px]'>
                        <Link to={'/'} className={`${currentPath === '/' ? 'text-white' : 'text-[#4d4d4d]'} text-[14px] cursor-pointer`}>brand page</Link>
                        <Link to={'/focus-page'} className={`${currentPath.includes('/focus-page') ? 'text-white' : 'text-[#4d4d4d]'} text-[14px] cursor-pointer`}>focus page</Link>
                        <Link to={'/more'} className={`${currentPath.includes('/more') ? 'text-white' : 'text-[#4d4d4d]'} text-[14px] cursor-pointer`}>more</Link>

                        {/* <span className={`${currentPath === '/' ? 'text-white' : 'text-[#4d4d4d]'} text-[14px] cursor-pointer`}>brand page</span> */}
                        {/* <span className={`${currentPath === '/focus-page' ? 'text-white' : 'text-[#4d4d4d]'} text-[14px] cursor-pointer'`}>focus page</span> */}
                        {userArticle?.unpublish ? (
                            <div
                                className="flex items-center text-[#FF2300] text-[14px] font-semibold cursor-pointer"
                                onClick={() => updateUserArticle(false)} // Call the function to update after changing state
                            >
                            ...
                            <div
                                className="flex items-center text-[#FF2300] text-[14px] font-semibold cursor-pointer"
                                onClick={() => updateUserArticle(true)} // Call the function to update after changing state
                            >
                                <DotSvg width={6} height={6} color={"red"} className="mr-2" />
                                <span>unpublish</span>
                            </div>
